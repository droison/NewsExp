...
import xyz.chaisong.newsexp.util.QLog;
...
public class ActivityLifecycleMgr extends MManager implements Application.ActivityLifecycleCallbacks {
    ...
    public void onActivityResumed(Activity activity) {
        ...
        if (wasBackground) {
            QLog.i(TAG, "went foreground");
            for (Listener l : listeners) {
                try {
                    l.onBecameForeground();
                } catch (Exception exc) {
                    QLog.e(TAG, "Listener threw exception!", exc);
                }
            }
        } else {
            QLog.i(TAG, "still foreground");
        }
    }
    ...
    private void logBackgroundState() {
        QLog.w(TAG, "当前进入后台：" + isBackground());
    }
}