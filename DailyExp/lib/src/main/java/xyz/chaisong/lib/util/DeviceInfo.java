...
public class DeviceInfo {
    ...
    public static String getArchType(Context context) {
        if (getSystemProperty(CPU_ARCHITECTURE_KEY_64, "").length() > 0) {
            if (LOGENABLE) {
                CLog.w("###############getSystemProperty","CPU arch is 64bit");
            }
            ...
            if (LOGENABLE) {
                CLog.w("###############getArchType()","return cpu DEFAULT 32bit!");
            }
    ...
    private static String getSystemProperty(String key, String defaultValue) {
        ...
        try {
            ...
            if (LOGENABLE) {
                CLog.w("getSystemProperty", "key = " + key + ", error = " + e.getMessage());
            }
        }

        if (LOGENABLE) {
            CLog.w("getSystemProperty",  key + " = " + value);
        }
        return value;
    }
        ...
        if (cpuInfo != null && cpuInfo.exists()) {
            ...
            try {
                ...
                if (line != null && line.length() > 0 && line.toLowerCase(Locale.US).contains("arch64")) {
                    if (LOGENABLE) {
                        CLog.w("###############isCPUInfo64()", PROC_CPU_INFO_PATH + " contains is arch64");
                    }
                    return true;
                } else {
                    if (LOGENABLE) {
                        CLog.w("###############isCPUInfo64()", PROC_CPU_INFO_PATH + " is not arch64");
                    }
                }
            } catch (Throwable t) {
                if (LOGENABLE) {
                    CLog.w("###############isCPUInfo64()","read " + PROC_CPU_INFO_PATH + " error = " + t.toString());
                }
            } finally {
                try {
    ...
    private static boolean isLibc64() {
        ...
        if (libcFile != null && libcFile.exists()) {
            ...
            if (header != null && header[EI_CLASS] == ELFCLASS64) {
                if (LOGENABLE) {
                    CLog.w("###############isLibc64()", SYSTEM_LIB_C_PATH + " is 64bit");
                }
                return true;
            }
        }
        ...
        if (libcFile64 != null && libcFile64.exists()) {
            ...
            if (header != null && header[EI_CLASS] == ELFCLASS64) {
                if (LOGENABLE) {
                    CLog.w("###############isLibc64()", SYSTEM_LIB_C_PATH_64 + " is 64bit");
                }
                return true;
            }
        }
            ...
            try {
                ...
                if (inputStream != null) {
                    ...
                    if (count == 16) {
                        ...
                        if (LOGENABLE) {
                            CLog.e("readELFHeadrIndentArray", "Error: e_indent lenght should be 16, but actual is " +  count);
                        }
                    }
                }
            } catch (Throwable t) {
                if (LOGENABLE) {
                    CLog.e("readELFHeadrIndentArray", "Error:" + t.toString());
                }
            } finally {
                if (inputStream != null) {
                    try {
