package xyz.chaisong.lib.network.response;
...
public class RespError extends VolleyError{
    ...
    private int mResponseErrorType;
    ...
    public int getResponseErrorType() {
        return mResponseErrorType;
    }

    public static RespError paramsError(String cause) {
        RespError error = new RespError(new Throwable(cause == null? "请求参数错误": cause));
        error.mResponseErrorType = ErrorType.paramsError;
        return error;
    }

    public static RespError notModifiedError() {
        RespError error = new RespError(new Throwable("304 notModified"));
        error.mResponseErrorType = ErrorType.responseNotModified;
        return error;
    }
    ...
    public static RespError responseNot200Error (String msg) {
        ...
        respError.mResponseErrorType = ErrorType.responseNot200Error;
        return respError;
    }

    public static RespError responseNeedLogin (String msg) {
        RespError respError = new RespError(msg);
        respError.mResponseErrorType = ErrorType.responseNeedLogin;
        return respError;
    }
}